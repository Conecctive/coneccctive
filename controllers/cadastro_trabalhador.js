...
router.get('/', (req, res) => {
  res.render('admin/login/cadastroTrabalhador', { layout: 'cadastroTrabalhador' })
})
  ...
  router.post('/', async (req, res) => {
    ...
    const emailVerificacao = await db.cadastro_trabalhador.findOne({
      where: {
        nm_email: data.email,
      },
    });
    ...
    const verificacaoCpf = await db.cadastro_trabalhador.findOne({
      where: {
        nr_cpf: data.cpf,
      },
    });
    ...
    if(senha !== senha2){
        return res.render('admin/login/cadastroTrabalhador', {
            layout: 'cadastroTrabalhador',
            data: req.body,
    ...
    if (emailVerificacao) {
      ...
      if (verificacaoCpf) {
        ...
        data.nm_trabalhador = req.body.nome
        ...
        db.cadastro_trabalhador.create(data).then(async (usuario) => {
            // Verificar o email
            ...
            transport.sendMail(mensagem, (erro) => {
              if (erro) {
                  ...
                  req.flash('erro',
                      'Usuário cadastrado com sucesso, porém o e-mail de confirmação não foi enviado, entre em contato com o e-mail'
                      + process.env.EMAIL_EMPRESA + '.')
                  res.redirect('/login')
              } else {
                  // Mensagem cadastrado com sucesso e-mail enviado com sucesso
                  req.flash('sucesso_msg', 'Usuário cadastrado com sucesso, confirme o e-mail na sua caixa de entrada! ')
                  res.redirect('/login')
        ...
        console.log(erro)
        return res.render('admin/login/cadastroTrabalhador',
         {layout: 'cadastroTrabalhador.handlebars', data, erro_msg: "Erro: não foi possivel cadastrar o usuário!"})
    })
    }}
...
router.get('/conf-email/:key', async (req, res) => {
    ...
    const usuario = await db.cadastro_trabalhador.findOne({
    ...
    if (usuario) {
        ...
        await db.cadastro_trabalhador.update({
            confEmail: null,
            id_situacaoConfirmar: 1
        }, {
            where: {
                id_trabalhador: usuario.id_trabalhador
            }
        }).then(() => {
